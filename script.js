...
async function fetchUniqueValues() {
  ...
  populateDropdown("subject", uniqueValues.subjects);
  document.getElementById("subject").selectedIndex = 1; // Set the first subject as selected by default
}
...
async function generateQuestions() {
  ...
  questions = data.data.slice(0, questionLimit); // Apply the limit on the client side
  showExamContainer();
}
          ...
          <svg viewBox="0 0 36 36" class="circular-chart5">
            ...
            <path
              ...
              style="stroke-dasharray: ${Math.floor(
                (subjectTimeSpent / 60 / (subjectQuestions.length * 2)) * 100
              )}, 100;"
            />
          </svg>
        </div>
